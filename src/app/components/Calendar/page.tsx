...
const Calendar: React.FC<CalendarProps> = ({ onSelectDate }) => {
  ...
  const getEmptyCells = (dayOfWeek: string) => {
    const emptyCells = [];
    const startIndex = weekDays.indexOf(dayOfWeek);
  ...
  return (
    <div className="grid grid-cols-6 gap-1 p-1">
      {months.map((month, index) => {
        ...
        const firstDayOfMonth = new Date(2022, index, 1).getDay(); // Get the day of the week for the first day of the month
        const emptyCells = getEmptyCells(weekDays[firstDayOfMonth-1]);
        return (
          <div key={month} className="border border-gray-300 rounded-lg p-3 shadow-xl shadow-emerald-900">
            ...
            <div className="grid grid-cols-7 ">
              ...
              {Array.from({ length: numDays }, (_, day) => day +1).map(day => {
                const dayOfWeek = getDayOfWeek(2022, index, day);
                return (
                  <div
                    key={day}
                    className="text-sm text-center m-[1px] py-1 cursor-pointer border shadow-2xl hover:border-black rounded"
